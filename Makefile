...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "acs", "tests")); system.time(test_check("acs", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
